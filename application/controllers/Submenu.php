...
class Submenu extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // Get max order
            $this->db->select_max('submenu_order');
            $max_order = $this->db->get('user_sub_menu')->row_array();
            $new_order = ($max_order['submenu_order'] ?? 0) + 1;
    ...
    public function update_submenu_order()
    {
        ...
        if (!empty($order)) {
            foreach ($order as $index => $id) {
                $this->db->where('id', $id);
                $this->db->update('user_sub_menu', ['submenu_order' => $index + 1]);
            }

            echo json_encode(['status' => 'success', 'message' => 'Submenu order updated successfully']);
        } else {
