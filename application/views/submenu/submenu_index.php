    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-hover table-lg" id="submenuTable">
                    <thead>
                        <tr>
                            <th scope="col">No</th>
                    ...
                    <tbody id="submenuTableBody">
                        <?php $i = 1 ?>
                        <?php foreach ($submenu as $sm) : ?>
                            <tr data-id="<?= $sm['id']; ?>" style="cursor: move;">
                                <th class="col-auto">
                                    <i class="bi bi-grip-vertical"></i> <?= $i; ?>
                                </th>
                                <td class="col-auto">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Initialize Sortable
        const submenuTableBody = document.getElementById('submenuTableBody');

        if (submenuTableBody) {
            const sortable = Sortable.create(submenuTableBody, {
                animation: 150,
                handle: '.col-auto',
    ...
    // Function to update submenu order
    function updateSubmenuOrder() {
        const rows = document.querySelectorAll('#submenuTableBody tr');
        const order = [];

        rows.forEach((row, index) => {
            order.push(row.getAttribute('data-id'));
            // Update row number display
            row.querySelector('th').innerHTML = `<i class="bi bi-grip-vertical"></i> ${index + 1}`;
        });

        // Send AJAX request to update order
        $.ajax({
            url: `${baseUrl}submenu/update_submenu_order`,
            method: 'POST',
            data: { order: order },
            success: function(response) {
                ...
                if (result.status === 'success') {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Success',
                        text: 'Submenu order updated successfully',
                        timer: 1500,
            ...
            error: function(xhr, status, error) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Failed to update submenu order'
                });
            }
        });
    }
